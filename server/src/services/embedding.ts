...
Only set semanticQuery when there is a descriptive or conceptual part that cannot be expressed as a filter.`,
    ...
    response_format: zodResponseFormat(QueryParseSchema, "query_parse"),
  });
  return response.choices[0]!.message.parsed!;
}

function rrf(lists: { id: string }[][], k = 60): string[] {
...
export async function hybridSearchActivities(userId: string, query: string) {
