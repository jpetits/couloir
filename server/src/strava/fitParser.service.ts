...
import { distance, lineString, point } from "@turf/turf";
import FitParser from "fit-file-parser";
import type { FitRecord, ParsedActivity, ParsedPoint } from "../types/types";
...
export class FitParserService {
  constructor(private readonly stravaParserService: StravaParserService) {}

  getStatsFromPoints(points: ParsedPoint[]) {
  ...
  getPointsFromRecords(records: FitRecord[]) {
    const filtered = records.filter((r) => r.position_lat && r.position_long);

    let cumDist = 0;

    return this.stravaParserService
      .simplifyByMaxDistance(
        filtered.map((r) => ({
          ...r,
  ...
  parseFitFile(fileBuffer: Buffer): Promise<ParsedActivity> {
    return new Promise((resolve, reject) => {
      fitParser.parse(fileBuffer as Buffer<ArrayBuffer>, (error, data) => {
        ...
        resolve({
          ...
          minSpeed:
            session?.max_speed ?? Math.min(...points.map((p) => p.speed)),
          maxElevation:
