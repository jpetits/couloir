...
export class ActivityRepository {
  ...
  findById(id: string) {
    return this.db.query.activities.findFirst({
  ...
  create(data: typeof activities.$inferInsert) {
    return this.db.insert(activities).values(data).returning();
  }
  delete(id: string) {
    return this.db.delete(activities).where(eq(activities.id, id));
  }
  ...
  findByStravaId(stravaActivityId: string, userId: string) {
    return this.db.query.activities.findFirst({
      where: and(
  ...
  findStravaIdsByUser(userId: string) {
    return this.db.query.activities.findMany({
      where: eq(activities.userId, userId),
  ...
  createMany(activitiesData: (typeof activities.$inferInsert)[]) {
    if (activitiesData.length === 0) return [];

    return this.db.insert(activities).values(activitiesData).returning();
  }

  search(userId: string, query: string, similarityScore: SQL<unknown>) {
  ...
  searchByFilters(userId: string, filters: Partial<ActivityFilters>) {
    const conditions = [eq(activities.userId, userId)];
    for (const key in filtersList) {
      const filter = filters[key as keyof ActivityFilters];
      if (filter !== undefined) {
